...
def plot_heatmaps(config: dict, results: list, instance=0, seq=0):
    ...
    path = os.path.join(config['global']['path'], config['global']['reports_path'])
    ...
    for i in range(pressure_levels):
        # Prediction u10
        axs[i, 0].imshow(predictions[seq, i, :, :, 0], cmap='hot', interpolation='nearest')
        axs[i, 0].axis('off')

        # Prediction v10
        axs[i, 1].imshow(predictions[seq, i, :, :, 1], cmap='hot', interpolation='nearest')
        axs[i, 1].axis('off')

        # Ground u10
        axs[i, 2].imshow(ground_truth[seq, i, :, :, 0], cmap='hot', interpolation='nearest')
        axs[i, 2].axis('off')
        ...
        axs[i, 0].text(-0.05, 0.5, f'Pressure Level {i+1}', va='center', ha='right',
                       rotation=90, transform=axs[i, 0].transAxes, fontsize=12)

    plt.tight_layout()
    if (config["plots"]["pred_vs_true"]):
        path = os.path.join(path, 'heatmaps_inst_{}_seq_{}.png'.format(instance, seq))
        plt.savefig(path)
    plt.show()
