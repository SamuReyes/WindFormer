...
class ViViT(nn.Module):
    ...
    def __init__(self, image_size_2d, patch_size_2d, seq_len, dim=192, depth=4, heads=3, dim_head=64, dropout=0., emb_dropout=0., reconstr_dropout=0., scale_dim=4):
        ...
        # Patch embedding layers for 2D data
        self.to_patch_embedding = PatchEmbedding(self.image_size_2d, patch_size_2d, dim)
    ...
    def forward(self, x_2d):
        ...
        x = self.to_patch_embedding(x_2d)  # [B, T, N, D]
        ...
        # Patch recovery
        surface_output = self.patch_recovery(x, b, t, self.to_patch_embedding.num_patches)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b t w h c -> (b t) c w h')  # [B*T, C, W, H]

        x = self.conv_surface(x)  # [B*T, D, W//patchW, H//patchH]

        x = rearrange(x, '(b t) d w h -> b t (w h) d', b=b, t=t)  # [B, T, N, D]
...
class PatchRecovery(nn.Module):
    def __init__(self, image_size_2d, patch_size_2d, dim):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, b, t, n_upper):
        ...
        x = rearrange(x, 'b t n d -> (b t) n d')  # [B*T, n_surface, D]

        x = x.permute(0, 2, 1)  # [B*T, D, n_surface]

        x = x.view(b*t, self.dim, *self.patch_shape_2d)  # [B*T, D, W//patchW, H//patchH]

        output = self.tconv_surface(x)   # [B*T, C, W, H]

        output = self.conv_refine_surface(output)  # [B*T, C, W, H]

        output = rearrange(output, '(b t) c w h -> b t w h c', b=b, t=t)  # [B, T, W, H, C]
